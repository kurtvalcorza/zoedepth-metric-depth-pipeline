...
`Intel/zoedepth-nyu-kitti` is the Transformers-format release of the ZoeDepth model fine-tuned jointly on NYU Depth v2 and KITTI (the "ZoeD-M12-NK" configuration), from "ZoeDepth: Zero-shot Transfer by Combining Relative and Metric Depth" (Bhat, Birkl, Wofk, Wonka, Müller, arXiv:2302.12288), converted by the Hugging Face team and pinned here to revision `f364d4c7936e91f465abba182208dd68142bf0ca` (the Hub's `main` on 2026-09-14). The snapshot declares a BEiT-large encoder with a DPT fusion decoder and two metric bins heads selected by a latent domain classifier. At inference the processor encodes the image, the model emits depth in metres, and post-processing restores the input size. This repository adds verified packaging and an optional bounded adaptation route: `freeze_for_adaptation` leaves only `metric_head.*` trainable; `finetune` uses caller-owned RGB/metric-depth records with explicit AdamW settings; `evaluate_adaptation` reports valid-pixel-weighted held-out `abs_rel` and `delta1` beside a constant training-median baseline; and `save_artifact`/`from_artifact` export and reload only that surface through a digest-bound SafeTensors artifact.
...
- Tutorial execution: exact carrier commit `ac175a5` / blob `3d3254ea89e1` passed all 11 code cells on a clean Kaggle T4 after one expected dependency-install restart; the bounded adaptation path, held-out evaluation, SafeTensors export, and fresh-base reload all completed. See `docs/release-verification.md`.
