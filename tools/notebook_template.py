    ...
    "cells": [
        ...
        },
        {
            ...
            "code": (
                ...
                "    model_abs_rel, model_delta1, base_abs_rel, base_delta1 = [], [], [], []\n"
                "    for record in records:\n"
                "        prediction = model_pipe.predict(record['image'])['depth']\n"
                "        reference = record['depth_m']\n"
                ...
                "        base_abs_rel.append(abs_rel(baseline, reference))\n"
                "        base_delta1.append(delta1(baseline, reference))\n"
                "    return {{'records': len(records), 'abs_rel': float(np.mean(model_abs_rel)), 'delta1': float(np.mean(model_delta1)), 'constant_median_baseline_abs_rel': float(np.mean(base_abs_rel)), 'constant_median_baseline_delta1': float(np.mean(base_delta1))}}\n\n"
